...
// 相关帖子 (same zone; same type or a shared tag; ≤ 4) as SpotlightCard tiles.
  ...
  return (
    <section className={className} aria-label={t('post_related')}>
      ...
      <ul className="grid gap-3 sm:grid-cols-2">
        {posts.map((p) => {
          const Icon = POST_TYPE_ICONS[p.type];
          return (
            <li key={p.id}>
              <SpotlightCard className="h-full">
                <Link href={zonePostHref(p.zone.slug, p.id)} className="flex h-full flex-col gap-2 p-4 outline-none focus-visible:ring-2 focus-visible:ring-zinc-400">
                  <span className="inline-flex items-center gap-1 self-start rounded-full border border-zinc-300 px-2 py-px text-[11px] font-medium text-zinc-700 dark:border-zinc-700 dark:text-zinc-300">
                    <Icon className="h-3 w-3" />
                    {tl(`zonePostType.${p.type}`)}
                  </span>
                  ...
                  <span className="mt-auto flex flex-wrap items-center gap-x-3 gap-y-1 pt-1 text-[11px] text-muted">
                    <span className="inline-flex items-center gap-1.5">
                      <Avatar name={p.author.displayName} src={p.author.avatarUrl} size="xs" tone="neutral" />
