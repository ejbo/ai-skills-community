...
/**
 * 笔记 side panel: the viewer's own highlights/notes (view / edit / delete /
 * locate) plus other readers' shared annotations, with a per-user visibility
 * filter (只看 / 隐藏 / 恢复). All community content stays HERE — the page
 * itself only carries calm markers.
 */
export function NotesPanel({
  ...
  useEffect(() => {
    ...
    if (!open || !own || appliedNoteRef.current === editNoteId) return;
  ...
  useEffect(() => {
    if (!open || !focusNoteId || !communityNotes) return;
  ...
  }, [open, focusNoteId, communityNotes]);
  ...
  return (
    <SidePanel side="right" title={t('notes_title')} onClose={onClose} widthClass="lg:w-[360px]">
      <div ref={listRef} className="min-h-0">
        ...
        {/* own highlights */}
        <h3 className={sectionHead}>{t('my_notes_count', { count: own?.length ?? 0 })}</h3>
        {own === null ? (
                    ...
                    <li key={hl.id} className="space-y-2 px-4 py-3">
                      ...
                      <div className="flex items-center gap-3 text-[11px]">
                        ...
                        <time className="r-muted ml-auto">
                          {relativeTime(hl.createdAt, locale)}
                        </time>
                      </div>
                    </li>
                  ))}
                  ...
                  <ul className="divide-y divide-[color:var(--reader-border)]">
                    {list.map((note) => (
                      ...
                      >
                        ...
                        {note.noteText && (
                          <p className="mt-2 text-sm leading-relaxed">{note.noteText}</p>
                        )}
