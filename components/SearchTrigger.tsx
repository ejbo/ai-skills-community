...
export function SearchTrigger() {
  ...
  // ⌘K / Ctrl+K toggles the palette. Accepting either modifier avoids a
  // navigator.platform sniff (and its hydration-sensitive initial state).
  useEffect(() => {
  ...
  return (
    <>
      {/* Icon-only until lg so the whole bar still fits a 14" laptop; the
          ⌘K/Ctrl+K shortcut stays live, it just isn't advertised. */}
      <button
        onClick={() => setOpen(true)}
        aria-label={t('search_placeholder')}
        className="flex h-9 shrink-0 items-center gap-2 rounded-lg border border-zinc-200 bg-white px-2.5 text-sm text-zinc-500 transition hover:border-zinc-300 hover:text-zinc-700 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-400 dark:hover:border-zinc-700 dark:hover:text-zinc-200 lg:w-40 lg:px-3 xl:w-44"
      >
        <Search className="h-3.5 w-3.5 shrink-0" />
        <span className="hidden truncate lg:inline">{t('search_placeholder')}</span>
      </button>
